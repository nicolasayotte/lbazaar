import { bytesToText, MintingPolicyHash, UTxO } from '@hyperionbt/helios';

export { getTokenNamesAddrs, toSchema };

const toSchema = (a) => {
  if (Array.isArray(a)) return a.map((val) => toSchema(val));
  if (typeof a === 'object')
    return {
      map: Object.entries(a).map(([key, val]) => [key, toSchema(val)]),
    };
  return a;
};
