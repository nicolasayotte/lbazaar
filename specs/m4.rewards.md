...
See `specs/glossary.md` for full glossary. Epic-specific additions below:
...
See `specs/glossary.md` for shared constraints. Epic-specific constraints below:
...
### F-18: Student Self-Mint — Implementation Status

**Status:** Functional (CIP-30 double-sign flow)

**Architecture:**
1. Student clicks "Mint" on Badges page → `RewardsTable.jsx::handleCip30Mint()`
2. Frontend gets UTXOs + change address from CIP-30 wallet
3. `POST /wallet/build-mint-tx` → `StudentMintController::buildMintTx()` → calls `build-student-certificate-tx.mjs` or `build-student-token-tx.mjs`, owner-signs tx
4. Student co-signs via `walletAPI.signTx(cbor, true)` (partial sign — returns `TransactionWitnessSet`, not full `Transaction`)
5. `POST /wallet/submit-mint-tx` → `StudentMintController::submitMintTx()` → calls `submit-signed-tx.mjs` which merges witness sets server-side and submits via Blockfrost
6. On success, `CertificateService::updateCertificateStatus()` records `self_minted` status + tx hash

**Key design decision:** CIP-30 `signTx(tx, partial=true)` returns a `TransactionWitnessSet` per spec, not a full `Transaction`. The witness merge must happen server-side (not via `walletAPI.submitTx`), because the browser only has the student's witness set and cannot merge it with the owner's.

**DB Schema:**
`course_histories.certificate_status` ENUM includes `self_minted` (added in migration `2026_03_10_000001`). This distinguishes student-initiated mints from teacher/admin airdrops (`minted`). Both statuses set `certificate_minted_at`.

**Fallback (no wallet):** `handleServerMint()` calls `/classes/{id}/attend/{schedule_id}/self-mint` for custodial/server-side minting (no CIP-30 required).

### F-10: Reward Airdrop — Implementation Status

**Status:** Core tx-building chain functional. Platform wallet pays for airdrop minting.

**Architecture:**
1. Teacher clicks "Mint Certificate" on ManageClass → `CertificateService::mintAndAirdropCertificate()`
2. PHP resolves student wallet address, builds metadata, calls `build-certificate-tx.mjs` via `exec()`
3. `build-certificate-tx.mjs` imports `fetchUtxos()` from `get-owner-utxos.mjs` to get platform wallet UTXOs from Blockfrost, builds unsigned tx with NFT mint + recipient output + reference token output
4. `submit-certificate-tx.mjs` signs with platform owner key (account 0) and submits via Blockfrost
5. On success, `CertificateService::updateCertificateStatus()` records `minted` status + tx hash

**Key design decisions:**
- Platform wallet (`OWNER_WALLET_ADDR`) pays all minting fees for teacher-initiated airdrops (not the teacher's wallet)
- `get-owner-utxos.mjs` fetches real UTXOs from Blockfrost and returns Helios `TxInput` CBOR hex (same format as CIP-30 wallets)
- `build-certificate-tx.mjs` directly imports `fetchUtxos()` as a module (no subprocess shelling)
- Reference token (CIP-68 `(100)` prefix) sent back to platform wallet; user token (`(222)` prefix) sent to student
