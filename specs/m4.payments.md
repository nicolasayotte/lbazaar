...
- ~~**OQ-04**: Resolved — preprod (NETWORK=preprod, BLOCKFROST_API_KEY=preprod*). Protocol parameters in `web3/config/preprod.json` must be kept current (see Implementation Notes).~~
...
### F-02: Purchase Class with ADA — Implementation Status

**Status:** Functional (CIP-30 double-sign flow)

**Architecture:**
1. Student connects CIP-30 wallet → UTXOs + change address sent to backend
2. `CoursePurchaseService::buildPurchaseTransaction()` → calls `build-purchase-tx.mjs` which builds tx, owner-signs it
3. Student co-signs via `walletAPI.signTx(cbor, true)` (partial sign)
4. `CoursePurchaseService::submitPurchaseTransaction()` → calls `submit-purchase-tx.mjs` which merges witness sets and submits via Blockfrost

**Address Resolution (recipients):**
Recipients (teacher, admin) receive funds to custodial addresses, not CIP-30 browser wallet addresses. `resolvePaymentAddress()` derives the address from the user's DB ID via `get-custodial-address.mjs` (HD derivation from platform ENTROPY). The CIP-30 wallet address is only used for *spending* (the student's change address), never for receiving. Admin commission goes to `OWNER_WALLET_ADDR` from `.env`.

**Protocol Parameters:**
`web3/config/preprod.json` contains Helios `NetworkParams` used by all tx-building scripts. Must match the current on-chain parameters or transactions fail with `PPViewHashesDontMatch`. Updated to Conway era (protocol v10.0, epoch 275) on 2026-03-10. Refresh by fetching from Blockfrost: `GET /epochs/latest/parameters` and `GET /blocks/latest`.
